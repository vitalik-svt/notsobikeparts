...
export default function CardNavButton({ direction, onClick }: Props) {
    ...
    return (
        <button
            type="button"
            className={`absolute top-27 ${direction === 'prev' ? 'left-0' : 'right-0'} text-white z-50 w-15 h-15 flex justify-center items-center border-2 border-black cursor-pointer bg-white lg:top-1/2 lg:-translate-y-1/2`}
            onClick={onClick}
